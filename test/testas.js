...
describe('Project', () => {
    ...
    //--- Test-2  OK
    it(' Test-2 should Project to have requared data (_id, projectName, projectPrice  and etc.)', (done) => {
        chai.request(app)
            .get('/projects/getAll')
            .end((err, response) => {
                chai.request(app)
    ...
    //--- Test-3 
    it(' Test-3 should return list of projects', (done) => {
        chai.request(app)
            .get('/projects/getAll')
            .end((err, res) => {
    ...
    //-- Test-4
    it(' Test-4 should add a new project ', (done) => {
        chai.request(app)
            .post('/projects/new')
            .send({
                projectName: "P3",
    ...
    // //--- Test-5
    // it(' Test-5 should update a projectNamme', function (done) {
    //     chai.request(app)
    //         .get('/projects/getAll')
    ...
    //                     res.should.have.status(200);
    //                     const proj = res.body
    //                     // proj[0].should.have.property('projectName')
    //                     // proj[0].should.have.property('_id');
    //                     console.log('project   :' + proj[0].projectName)
    //                     //proj.should.have.property('projectName');
    //                     //proj.projectName.should.equal('P1 updated');
    //                     //         project.projectPrice.should.not.equal(((response.body.length) - 1));
    //                     //         project.projectAddress.Street.should.not.equal(null);
    //                     //         project.projectAddress.City.should.not.equal(null);
    //                     done();
    ...
    // --- Teast-6
    it(' Test-6 should delete a project', function (done) {
        chai.request(app)
            .get('/projects/getAll')
            .end(function (err, response) {
                chai.request(app)
